prism.registerWidget("parallelSets", {
            name: "Parallel Sets"
            , family: "Sankey"
            , title: "parellelSets"
            , hideNoResults: true
            , iconSmall: "/plugins/parSets/widget-24.png"
            ...
            , data: {
                ...
                , canColor: function (widget, panel, item) {
                    return (panel.name === 'Node Ids') || (panel.name === 'Node Labels');
                }
                , allocatePanel: function (widget, metadataItem) {
                    // measure
                    if (prism.$jaql.isMeasure(metadataItem) && widget.metadata.panel("Node Ids").items.length === 0) {
                        return "Node Ids";
                    }
                ...
                populateMetadata: function (widget, items) {
                    ...
                    widget.metadata.panel("Categories").push(a.dimensions);
                    // allocating dimensions
                    widget.metadata.panel("Node Ids").push(a.dimensions);
                    widget.metadata.panel("Node Labels").push(a.dimensions);
                    // allocating filters
                ...
                buildQuery: function (widget) {
                    ...
                    //	Get the panel items
                    var categories = widget.metadata.panel("Categories").items;
                    var nodeIds = widget.metadata.panel("Node Ids").items;
                    var nodeLabels = widget.metadata.panel("Node Labels").items;
                    var nodeValues = widget.metadata.panel("Values").items;
                    //	Only run if there are equal number IDs and Labels
                    if (nodeIds.length === nodeLabels.length) {
                        //pushing all categories
                        for (i = 0; i < categories.length; i++) {
                            query.metadata.push(categories[i]);
                        }
                        // pushing all nodes
                        for (i = 0; i < nodeIds.length; i++) {
                            //	Add the id
                            query.metadata.push(nodeIds[i]);
                            //	Add the label
                            query.metadata.push(nodeLabels[i]);
                        }
                        //	adding the values
                        for (i = 0; i < nodeValues.length; i++) {
                            query.metadata.push(nodeValues[i]);
                        }
                        // pushing filters
                    ...
                    return query;
                }, //create tree hierchy data structure
                processResult: function (widget, queryResult) {
                    ...
                    var colors = prism.activeDashboard.style.palette();
                    //	Get the metadata
                    var nodeMetadata = queryResult.metadata();
                    ...
                    for (var i = 0; i < n; i++) {
                        ...
                        , for (var j = 0; j < nd; j++) {
                            var dimension = dimensions[j]
                                ...
                                , children = partree.children;
                            partree.count += v;
                            partree = children.hasOwnProperty(category) ? children[category] : children[category] = {
            ...
            var options = {
                showLabels: widget.style.isCurved
            };
            ...
            //	Define number/color formatter
            //Address JAQL build according to Parsets
            var valueItem = widget.metadata.panel("Values").items[0]
                , formatPrefix = valueItem.jaql.title
                , formatMask = valueItem.format.mask
                , formatNumber = prism.$injector.get('$filter')('numeric'), //formatNumber = d3.format(",.0f"),
                format = function (d) {
                    return formatPrefix + ': ' + formatNumber(d, formatMask);
                }
                , color = d3.scale.category20();
            //	Get the data to render
            var data = widget.queryResult;
            ...
            var categoriesArray = function () {
                ...
                for (var i in partree) {
                         proxy[i]=partree.name;
                    }
                //populate unique array with unique values derived from proxy array
                for(var j in proxy){
                    var found=false;
                    for(var k in unique){
                         if(proxy[j]===unique[k]){
            ...
            return unique;
                ...
                //load Parsets instance
            var chart = d3.parsets().dimensions(categoriesArray); 
            ...
            g.selectAll(".ribbon, .ribbon-mouse").data(["ribbon", "ribbon-mouse"], String).enter().append("g").attr("class", String); //Fix
            updateDimensions();
            if (tension != tension0) {
            ...
            function updateDimensions() {
                ...
                g.select(".ribbon").selectAll("path").each(function (d) {
                    ...
                    for (var i = 0; i < n; i++) {
                        var p = path[i];
                        node = node.children.hasOwnProperty(p) ? node.children[p] : node.children[p] = {
                            children: {}
                            , count: 0
                        };
                    }
                ...
                dimensions.forEach(function (d) {
                        ...
                        dimension.call(d3.behavior.drag().origin(identity).on("dragstart", function (d) {
                            dragging = true;
                            d.y0 = d.y;
                        }).on("drag", function (d) { //Crucuial
                            d.y0 = d.y = d3.event.y;
                            for (var i = 1; i < dimensions.length; i++) {
                                if (height * dimensions[i].y < height * dimensions[i - 1].y) {
                                    ...
                                    ordinal.domain([]).range(d3.range(dimensions[0].categories.length));
                                    nodes = layout(tree = buildTree({
                                        children: {}
                                        , count: 0
                                    }, data, dimensionNames, value_), dimensions, ordinal);
                                    total = getTotal(dimensions);
                                    g.selectAll(".ribbon, .ribbon-mouse").selectAll("path").remove();
                                    updateRibbons();
                                    updateCategories(dimension);
                                    dimension.transition().duration(duration).attr("transform", translateY).tween("ribbon", ribbonTweenY);
                        ...
                        dimension.select("text").select("tspan.sort.alpha").on("click.parsets", sortBy("alpha", function (a, b) {
                            return a.name < b.name ? 1 : -1;
                        }, dimension));
                        dimension.select("text").select("tspan.sort.size").on("click.parsets", sortBy("size", function (a, b) {
                            return a.count - b.count;
                        }, dimension));
                        dimension.transition().duration(duration).attr("transform", function (d) {
                            return "translate(0," + d.y + ")";
                    ...
                    function updateRibbons() {
                        ...
                        ribbon.enter().append("path").each(function (d) {
                        ...
                        mouse.enter().append("path").on("mousemove.parsets", function (d) {
                    ...
                    function updateCategories(g) {
                        ...
                        var categoryEnter = category.enter().append("g").attr("class", "category").attr("transform", function (d) {
                            return "translate(" + d.x + ")";
                        });
                        category.exit().remove();
                        category.on("mousemove.parsets", function (d) {
                            ribbon.classed("active", false);
                            if (dragging) return;
                            ...
                            d3.event.stopPropagation();
                        }).on("mouseout.parsets", unhighlight).on("mousedown.parsets", cancelEvent).call(d3.behavior.drag().origin(identity).on("dragstart", function (d) {
                            dragging = true;
                            d.x0 = d.x;
                        }).on("drag", function (d) {
                            d.x = d3.event.x;
                        ...
                        category.transition().duration(duration).attr("transform", function (d) {
                            return "translate(" + d.x + ")";
                        ...
                        category.select("rect").attr("width", function (d) {
                            return d.dx;
                        }).attr("class", function (d) {
                            return "category-" + (d.dimension === dimensions[0] ? ordinal(d.name) : "background");
                        });
                        category.select("line").attr("x2", function (d) {
                            return d.dx;
                        });
                        category.select("text").text(truncateText(function (d) {
                            return d.name;
                        }, function (d) {
                            return d.dx;
                        }));
          ...
          , "Z"]).join("");
                ...
                var percent = d3.format("%")
                    ...
                    , parsetsEase = "elastic"
                    , parsetsId = 0;
                //prob helps other function
                function identity(d) {
                    return d;
                }
                //this is for the translate attribute in the HTML element
                function translateY(d) {
                    return "translate(0," + d.y + ")";
                }
                //Recurses parents and binds them to path var for showing them with -> separator, the value, and percent out of origin
                ...
                //This is for each category, percent out of 100 in corresponding dimension and percent of same dimension
